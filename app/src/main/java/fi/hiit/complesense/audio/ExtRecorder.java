...
public class ExtRecorder 
{
	...
	private AudioRecord.OnRecordPositionUpdateListener updateListener = new AudioRecord.OnRecordPositionUpdateListener()
	{
		public void onPeriodicNotification(AudioRecord recorder)
		{
			if(state != State.RECORDING)
				Log.i(TAG, "updateListener() called @ state=" + state);
			...
			if (state == State.RECORDING){
                ...
                {
                    Log.e(TAG, e.toString()+ " @state: " + state);
                } catch (IOException e)
                {
                    Log.e(TAG, e.toString() + " @state: " + state);
                }
	...
	public ExtRecorder(boolean uncompressed, int audioSource, int sampleRate, int channelConfig, int audioFormat, WebSocket socket)
	{
		try
		{
			...
			{ // RECORDING_UNCOMPRESSED
				...
				Log.i(TAG,"AudioRecord init with sampleRate: " + sampleRate + " channel: " + channelConfig +
						" encoding: "+ audioFormat + " bufferSize: " + bufferSize);
	...
	*/
	...
	public void release()
	{
		Log.i(TAG,"Enter release() @ state = " + state);
		if (state == State.RECORDING)
		{
			stop();
		}
	...
	public void reset()
	{
		Log.i(TAG,"Enter reset() @ state = " + state);
		try
		{
	...
	public void start()
	{
		Log.i(TAG,"Enter start() @ state = " + state);
	...
	public void stop()
	{
		Log.i(TAG,"Enter stop() @ state = " + state);
		if (state == State.RECORDING)
		{
			if (rUncompressed)
			{
